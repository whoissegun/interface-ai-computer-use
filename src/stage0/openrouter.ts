...
function assistantMessage(value: unknown): Extract<ChatMessage, { role: "assistant" }> {
  ...
  // Preserve provider fields such as reasoning_details. Some providers require
  // those fields to be sent back unchanged on the next turn.
  return {
    ...raw,
    role: "assistant",
    content,
  ...
  async complete(request: ModelRequest): Promise<ModelResponse> {
    const response = await fetch(this.endpoint, {
      ...
      headers: {
        ...
        "X-Title": "Interface.ai Computer Use Stage 0"
      },
      body: JSON.stringify({
        model: request.model,
    ...
    if (!response.ok) {
      throw new Error(`OpenRouter HTTP ${response.status}: ${payload.error?.message ?? body.slice(0, 500)}`);
    }
